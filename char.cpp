...
void CHARACTER::Initialize()
{
	...
	m_ridingItemBeforeDeath = NULL;
	m_ridingMountVnumBeforeDeath = 0;
#endif
...
#ifdef __GROWTH_PET_SYSTEM__
	if (m_petGrowthSystem)
	{
		m_petGrowthSystem->Destroy();
		delete m_petGrowthSystem;
...
#endif
{
	...
	nTotalMoney += GetGold(); // static_cast<long long>(GetGold());
#ifdef __CHEQUE_SYSTEM__
...
void CHARACTER::Disconnect(const char* c_pszReason)
{
...
#ifdef __BATTLE_PASS_SYSTEM__
	ListBattlePassMap::const_iterator it = m_listBattlePass.begin();
	...
	if (GetDesc())
	{
#ifdef _IMPROVED_LOGOUT_POINTS_
...
#endif
#ifdef __ITEM_POWER_SYSTEM__
	pack.points[POINT_ITEM_POWER] = GetItemPower();
#endif
...
void CHARACTER::SetPlayerProto(const TPlayerTable* t)
{
	...
	m_petGrowthSystem = M2_NEW CGrowthPetSystem(this);
#endif
...
void CHARACTER::SetProto(const CMob* pkMob)
{
	...
	/*
	if (GetRaceNum() == TEMPLE_OCHAO_PORTAL)
...
#ifdef __ITEM_POWER_SYSTEM__
	if (IsPC() && GetItemPower() > 0)
		PointChange(POINT_ITEM_POWER, GetItemPower());
#endif
...
void CHARACTER::PointChange(BYTE type, long long amount, bool bAmount, bool bBroadcast)
{
	...
	case POINT_LEVEL_STEP:
		if (amount > 0)
		{
			...
			case 3:
				...
				if (GetLevel() <= gPlayerMaxLevelStats)
	...
	case POINT_GOLD:
	{
		const long long nTotalGold = static_cast<long long>(GetGold()) + static_cast<long long>(amount);
	...
	case POINT_CHEQUE:
	{
		...
		SetCheque(GetCheque() + (int64_t)amount);
		val = GetCheque();
	}
	break;
#endif
...
bool CHARACTER::WarpSet(long x, long y, long lPrivateMapIndex)
{
	...
	*/
...
#ifdef __SORT_INVENTORY_ITEMS__
	if (GetSortInventoryPulse() > thecore_pulse())
	{
...
#ifdef __SPECIAL_INVENTORY_SYSTEM__
	if (GetSortSpecialInventoryPulse() > thecore_pulse())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot warp after sorting your inventory."));
		return false;;
	}
	...
	if (p.wPort != mother_port /*config.cpp*/)
...
#ifdef __GROWTH_PET_SYSTEM__
DWORD CHARACTER::GetPetNextExp() const
{
	if (IsGrowthPet())
	{
		if (PLAYER_EXP_TABLE_MAX < GetLevel())
...
#ifdef __MOUNT_COSTUME_SYSTEM__
void CHARACTER::MountAfterDeath()
{
	LPITEM lastMountItem = RidingItemBeforeDeath();

	if (lastMountItem != NULL)
	{
		if (COSTUME_MOUNT == lastMountItem->GetSubType() && lastMountItem->GetType() == ITEM_COSTUME && IS_SET(lastMountItem->GetFlag(), ITEM_FLAG_QUEST_USE_MULTIPLE))
		{
			m_dwMountVnum = 0;
			quest::CQuestManager::instance().UseItem(GetPlayerID(), lastMountItem, false);
		}
	}
}
#endif
...
void CHARACTER::AddAcceMaterial(TItemPos tPos, BYTE bPos)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot use locked items."));
		return;
	}
#ifdef __SOUL_BIND_SYSTEM__
...
void CHARACTER::SortInventoryItems()
{
	...
	while (it != v.end())
	{
		...
		if (item)
		{
			...
			/// 2020.03.01.Owsap - Stack skill book inside special inventory after sorting
			// SORT_STACKED_ITEM_SKILLBOOK_FIX
...
#ifdef __EXTENDED_ITEM_COUNT__
							short bCount = MIN(ITEM_MAX_COUNT - book_item->GetCount(), item->GetCount());
#else
							BYTE bCount = MIN(200 - book_item->GetCount(), item->GetCount());
#endif
			...
			// END_OF_SORT_STACKED_ITEM_SKILLBOOK_FIX
			else
			{
...
void CHARACTER::SortSpecialInventoryItems(BYTE type)
{
	...
	if (IsDead() || GetExchange() || GetMyShop() || GetShopOwner() || IsOpenSafebox() || IsCubeOpen())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Please close other windows before sorting your special inventory."));
		return;
	}
	...
	while (it != vecItems.end())
	{
		...
		if (item)
		{
			...
			// SORT_STACKED_ITEM_SKILLBOOK_FIX
			bool bStackable = false;
			if (type == 0 /* SKILL_BOOK_INVENTORY_SLOT_START */)
			{
				if (item->GetType() == ITEM_SKILLBOOK || item->GetType() == ITEM_SKILLFORGET)
				{
					for (int i = SKILL_BOOK_INVENTORY_SLOT_START; i < SKILL_BOOK_INVENTORY_SLOT_END; ++i)
...
void CHARACTER::ChangeLookWindow(bool bOpen, bool bRequest)
{
	...
	sPacket.subheader = bOpen ? CL_SUBHEADER_OPEN : CL_SUBHEADER_CLOSE;
	sPacket.dwCost = bOpen ? CL_TRANSMUTATION_PRICE : 0;
	sPacket.bPos = 0;
	sPacket.tPos = tPos;
...
void CHARACTER::AddClMaterial(TItemPos tPos, BYTE bPos)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot do this while the item is equipped."));
		return;
	}
...
void CHARACTER::RefineClMaterials()
{
	...
	unsigned long long llPrice = CL_TRANSMUTATION_PRICE;
...
#ifdef __EXTENDED_ITEM_COUNT__
void CHARACTER::OpenMyOfflineShop(const char* c_pszSign, TOfflineShopItemTable* pTable, short bItemCount, int iTime, BYTE bNpcType)
#else
void CHARACTER::OpenMyOfflineShop(const char* c_pszSign, TOfflineShopItemTable* pTable, BYTE bItemCount, int iTime, BYTE bNpcType)
#endif
{
	...
	if (CountSpecifyItem(OFFLINE_SHOP_NEED_ITEM) == 0)
	{
		LPITEM pItem = ITEM_MANAGER::instance().CreateItem(OFFLINE_SHOP_NEED_ITEM);
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Offline Shop> You need %s in order to open a shop."), pItem->GetName());
		return;
	}
	...
	case 1:
		open_cost = 2500000;
		open_time = 86400; // 1 day
		break;
	case 2:
		open_cost = 5000000;
		open_time = 604800; // 7 days
		break;
	case 3:
		open_cost = 15000000;
		open_time = 1296000; // 15 days
		break;
	case 4:
		open_cost = 50000000;
		open_time = 2592000; // 30 days
		break;
	default:
		open_cost = 2500000;
		open_time = OFFLINE_SHOP_DEFAULT_TIME;
		break;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Offline Shop> You do not have enought gold to open a shop for %d hours."), (open_time / 60) / 60);
		return;
	}
	...
	snprintf(szInsertQuery, sizeof(szInsertQuery), "INSERT INTO offline_shop_npc%s (owner_id, sign, name, time, x, y, z, mapIndex, channel, level, align, guild, npc_vnum) VALUES(%u, '%s', '%s', %d, %ld, %ld, %ld, %ld, %d, %d, %d, '%s', %d)", get_table_postfix(), GetPlayerID(), szSign, GetName(), open_time, GetX(), GetY(), GetZ(), GetMapIndex(), g_bChannel, GetLevel(), GetAlignment(), GetGuild() ? GetGuild()->GetName() : "NONAME", npc_vnum);
	DBManager::Instance().DirectQuery(szInsertQuery);

	LPOFFLINESHOP pkOfflineShop = COfflineShopManager::instance().CreateOfflineShop(npc, GetPlayerID(), pTable, bItemCount, GetMapIndex(), open_time, szSign);
	if (!pkOfflineShop)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Offline Shop> Cannot open shop."));
	...
	ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You have opened a private offline shop for %d hours."), (open_time / 60) / 60);

	AddAffect(AFFECT_OFFLINE_SHOP, APPLY_NONE, 0, AFF_NONE, open_time, 0, false, false);
...
WORD CHARACTER::GetItemPower()
{
	...
	{
		LPITEM item = GetWear(i);
		if (item && item->GetPower() > 0)
			wItemPower += item->GetPower();
	}
